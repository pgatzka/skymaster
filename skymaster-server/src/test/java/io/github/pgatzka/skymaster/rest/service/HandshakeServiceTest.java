...
class HandshakeServiceTest {
    ...
    void handshakeDoesNotThrowWhenVersionsMatch() {
        ...
        HandshakeRequest request = new HandshakeRequest("d8cb5ee8-6607-4e26-8b2a-0f9e7c201a4e", "InternalError_", "1.0.0-SNAPSHOT");

        assertThatNoException()
                .isThrownBy(() -> handshakeService.handshake(request));
    }

    @Test
    void handshakeThrowsWhenVersionsDiffer() {
        when(buildProperties.getVersion()).thenReturn("1.0.0-SNAPSHOT");
        HandshakeRequest request = new HandshakeRequest("d8cb5ee8-6607-4e26-8b2a-0f9e7c201a4e", "InternalError_", "1.0.1-SNAPSHOT");

        assertThatThrownBy(() -> handshakeService.handshake(request))
